...
compressionam_index_fetch_end(IndexFetchTableData *scan)
{
	...
	elog(NOTICE, "num decompressions " INT64_FORMAT, cscan->num_decompressions);
	pfree(cscan);
}
								...
								bool *all_dead)
{
	...
	uint compr_index = tid->ip_blkid.bi_hi;
	...
	for (int i = 0; i < icstate->index_info->ii_NumIndexAttrs; i++)
	{
		...
		if (bms_is_member(table_attno, icstate->cdata->segmentby_cols))
		{
			...
			if (num_rows == 0)
			{
				...
				// elog(NOTICE, "segmentby count=%u", num_rows);
			}
		}
		else
		{
								...
								double *tuples, double *allvisfrac)
{
	...
	*tuples *= MAX_ROWS_PER_COMPRESSION;
	// elog(NOTICE, "%s compression pages %u tuples %lf", get_rel_name(crel->rd_id), *pages,
	// *tuples);
