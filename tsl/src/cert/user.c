 ...
 */
static void
make_user_cert_path(StringInfo result, const char *user_name, PathKind path_kind)
{
...
StringInfo
ts_user_cert_path(const char *user_name)
{
	StringInfo path = makeStringInfo();
	make_user_cert_path(path, user_name, PATH_KIND_CRT);
	return path;
}

StringInfo
ts_user_key_path(const char *user_name)
{
	StringInfo path = makeStringInfo();
	make_user_cert_path(path, user_name, PATH_KIND_KEY);
	return path;
}
...
{
	size_t copied pg_attribute_unused();

	result->roleid = get_role_oid(NameStr(*user_name), false);
	copied = strlcpy(NameStr(result->role_name), NameStr(*user_name), NAMEDATALEN);
	Assert(copied <= NAMEDATALEN);

	initStringInfo(&result->crt_file);
	make_user_cert_path(&result->crt_file, NameStr(*user_name), PATH_KIND_CRT);

	initStringInfo(&result->key_file);
	make_user_cert_path(&result->key_file, NameStr(*user_name), PATH_KIND_KEY);
}

TS_FUNCTION_INFO_V1(ts_pg_get_user_cert_info);

Datum ts_pg_get_user_cert_info(PG_FUNCTION_ARGS)
{
	...
	values[1] = CStringGetTextDatum(NameStr(cert_info.role_name));
	values[2] = CStringGetTextDatum(NameStr(cert_info.key_file));
	values[3] = CStringGetTextDatum(NameStr(cert_info.crt_file));
